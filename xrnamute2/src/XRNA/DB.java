...
public class DB {
	...
	// default values for connection to the XRNA.DB, may be replaced by
	// DB_PARAMS property file containing different values
	...
	/*
	 * Hard-coded queries to be used in inserting and retrieving data from the
	 * DB
	 */
	...
	final String INIT_RESULTS_TABLE = "CREATE TABLE IF NOT EXISTS RESULTS "
			+ "(id INT UNSIGNED NOT NULL, " + "result BLOB, "
		...
		try {
			...
			if (!PARAMS_LOADED) {
				...
				if (params.exists()) {
					...
					prop.load(in);
					if (!prop.isEmpty()) {
						dbName = prop.getProperty("db").isEmpty() ? dbName
								: prop.getProperty("db");
						user = prop.getProperty("user").isEmpty() ? user : prop
								.getProperty("user");
						password = prop.getProperty("password").isEmpty() ? password
	...
	/*
	 * Initialize the tables if they do not already exist, and purges rows older
	 * than 1 month from both tables if DB_PARAMS file is present in the
	 * directory, DB credentials may be retrieved from it
	 * 
	...
	public boolean initTables() {
		...
		try {
			...
			LOGGER.severe("Could not initialize the tables in the DB: "
					+ e.getMessage());
			ans = false;
		}
		return ans;
	 ...
	 * 
	 * @param temp - the temperature for the algorithm's calculation, default is
	 * 38
	 * 
	 * @param sequence - the RNA sequence for the algorithm
	 * 
	 * @return id - the row number (unique identifier) in the DB USER_INPUT
	 * table, where the data was saved if successful, 0 otherwise.
	 */
	public int insertInput(String email, String windowMin, String windowMax,
			String temp, String sequence) {
		Connection newCon = dbConnect();
		int id = 0; // default id is 0, will be replaced if insertion succeeds
		try {
			if (newCon != null) {
				st = newCon.prepareStatement(WRITE_INPUT,
	 ...
	 * 
	 * @return in - the inputData object representing the user input to be
	 * calculated if successful, an empty object otherwise
	 */
	public inputData getInput(int id) {
		Connection newCon = dbConnect();
		inputData in = new inputData(); // default is an empty object, will be
										// replaced if query succeeds
		try {
			if (newCon != null) {
				...
				if (rs.next()) {
					in = new inputData(id, rs.getString("email"),
							rs.getInt("windowMin"), rs.getInt("windowMax"),
							rs.getInt("temp"), rs.getString("sequence"));
				}
				rs.close();
				st.close();
				newCon.close();
			...
			LOGGER.warning("Could not get input from DB for id " + id + " : "
					+ e.getMessage());
		}
		return in;
	}
	 ...
	 * 
	 * @param id - the unique identifier for the row of the user input, to be
	 * used as the identifier for the correlating result
	 * 
	 * @param data - the resultData object representing the calculation results
	 * for bp distance and tree-edit distance for the given input
	 * 
	...
	public boolean insertResults(int id, resultData data, int status) {
		...
		try {
			...
			LOGGER.warning("Could not insert result to DB for id " + id + " : "
					+ e.getMessage());
		}
		return ans;
	}
	 ...
	 * 
	 * @param id - the unique identifier for the row of the result, matching the
	 * correlating input
	 * 
	 * @return res - the resultData object representing the calculation results
	 * for bp distance and tree-edit distance for the given input if successful,
	 * empty object otherwise
	 */
	public resultData getResults(int id){
        Connection newCon = dbConnect();
        resultData res = new resultData();
        res.getBpDist().add(new dataPoint(5,5,(float)0.7));//TODO take out
        	try{
            if(newCon!=null) {
                ...
                if (rs.next()) {
                	...
                	res.getBpDist().add(new dataPoint(70,70,(float)0.7));
                    Object x = ois.readObject();
                    res.getBpDist().add(new dataPoint(80,80,(float)0.7));
                    res = (resultData)x;
                }
                rs.close();
                st.close();
        	...
        	res.getBpDist().add(new dataPoint(90,90,(float)0.7));
	 ...
	 * correlating input
	 * 
	 * @return res - the int returned represents the job status - 0 if pending,
	 * 1 if successful, 2 if failed.
	 */
	public int checkResults(int id) {
		...
		try {
			...
			LOGGER.warning("Could not check result in DB for id " + id + " : "
					+ e.getMessage());
		}
		return res;
	}
